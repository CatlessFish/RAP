...
impl<'tcx> DeadlockDetection<'tcx> {
    pub fn lockset_analysis_1(&self) {
        // Steps:
                        ...
                        TerminatorKind::Call {
                            ...
                            if let Some((callee_def_id, _)) = func.const_fn_def() {
                                ...
                                if lock_apis.contains_key(&callee_def_id) {
                                    ...
                                    rap_info!("Calling on self = {:?}", args[0].node);
                                }
                            }
                        }
                        _ => {}
        ...
        // TODO: 结构化表示lock struct / api；怎么使用const / static lock objects
        // TODO: 整理出需要的数据结构
