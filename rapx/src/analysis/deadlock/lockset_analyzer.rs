...
pub struct FuncLockSetAnalyzerInner<'a> {
    ...
    analyzed_functions: &'a HashMap<DefId, FunctionLockSet>,
    func_lock_info: &'a std::cell::RefCell<FunctionLockSet>,
    callsites: &'a std::cell::RefCell<HashMap<Location, DefId>>,
}
        ...
        match &terminator.kind {
            TerminatorKind::Call {
                ...
                if let Some((callee, _args)) = func.const_fn_def() {
                    // 1. Record callsite
                    self.callsites.borrow_mut().insert(location, callee);
            ...
            TerminatorKind::Return => {
                ...
                let mut func_lock_info = self.func_lock_info.borrow_mut();
                if let Some(target_set) = func_lock_info.exit_lockset.get_mut(&self.call_context) {
...
impl<'tcx, 'a> FuncLockSetAnalyzer<'tcx, 'a> {
    ...
    pub fn run(&mut self) {
        ...
        let func_lock_info_cell = RefCell::new(self.func_lock_info.clone());
        let callsites_cell = RefCell::new(self.callsites.clone());
        let result = FuncLockSetAnalyzerInner {
            ...
            func_lock_info: &func_lock_info_cell,
            callsites: &callsites_cell,
        }
        .iterate_to_fixpoint(self.tcx, body, None);

        // Clone callsites to avoid longer reference
        let callsites = callsites_cell.clone().into_inner();
        ...
        self.func_lock_info = func_lock_info_cell.into_inner();
