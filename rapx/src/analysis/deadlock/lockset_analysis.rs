...
impl<'tcx> DeadlockDetection<'tcx> {
    pub fn lockset_analysis(&mut self) {
        ...
        rap_info!(
            "Collected {} lock objects",
            program_lock_info.lock_objects.len()
        );
        ...
        rap_info!(
            "Completed Lockset Analysis for {} functions",
            analyzed_count
        );
        ...
        for local_def_id in self.tcx.hir().body_owners() {
            ...
            for &(api_name, lock_state_str) in &self.target_lock_apis {
                if fn_name.contains(api_name) {
                    ...
                    program_lock_info
                        .lock_apis
                        .insert(def_id, (fn_name.clone(), lock_state.clone()));
                    rap_debug!(
                        "Found lock API: {:?}, lock state: {:?}",
                        fn_name,
                        lock_state
                    );
    ...
    fn analyze_function_lockset(
        &self,
        func_def_id: DefId,
        program_lock_info: &mut ProgramLockInfo,
    ) {
        /* filter const mir */
        ...
        while changed && iteration < max_iterations {
            ...
            rap_debug!(
                "Function {} Lockset Analysis Iteration #{}",
                func_name,
                iteration
            );
                ...
                if let Some(terminator) = &bb.terminator {
                    match &terminator.kind {
                        TerminatorKind::Call {
                            ...
                            if let Some(callee_func_def_id) = self.resolve_function_call(func) {
                                ...
                                for arg in args {
                                    ...
                                    if let Operand::Copy(r_place) | Operand::Move(r_place) =
                                        arg_operand
                                    {
                                        dependency_map
                                            .entry(l_place)
                                            .or_insert(HashSet::new())
                                            .insert(r_place.local);
                                    }
                                }

                                // Check if it's a lock API
                                if let Some((api_name, ..)) =
                                    program_lock_info.lock_apis.get(&callee_func_def_id)
                                {
                                    rap_debug!(
                                        "Found lock API: {} in function {}",
                                        api_name,
                                        func_name
                                    );

                                    // Try to determine the lock object being operated on
                                    if let Some(lock_def_id) = self.resolve_lock_object_from_args(
                                        &args,
                                        &local_lock_map,
                                        &dependency_map,
                                    ) {
                                        rap_debug!(
                                            "Lock API {} acts on lock object: {}",
                                            api_name,
                                            self.tcx.def_path_str(lock_def_id)
                                        );
                                        current_lockset
                                            .update_lock_state(lock_def_id, LockState::MustHold);
                                ...
                                // Record call site
                                func_info.call_sites.push((bb_idx, callee_func_def_id));
                            }
                        }
                        TerminatorKind::Drop { place, .. } => {
                            ...
                            if let Some(lock_def_id) = self.resolve_place_to_lockguard(
                                &place,
                                &local_guard_map,
                                &dependency_map,
                            ) {
                                ...
                                rap_debug!(
                                    "Detected lock {:?} released in function {}",
                                    self.tcx.def_path_str(lock_def_id),
                                    func_name
                                );
                                current_lockset
                                    .update_lock_state(lock_def_id, LockState::MustNotHold);
                            }
                        }
        ...
        program_lock_info
            .function_lock_infos
            .insert(func_def_id, func_info.clone());
    ...
    fn handle_assignment(
        ...
        match rvalue {
            Rvalue::Use(operand) => {
                ...
                if let Some(lock_def_id) =
                    self.resolve_operand_to_lock_object(operand, lock_map, dependency_map)
                {
                    lock_map.insert(place.local, lock_def_id);
                }
                if let Some(guard_def_id) =
                    self.resolv_operand_to_lockguard(operand, guard_map, dependency_map)
                {
            ...
            Rvalue::Ref(_, _, borrowed_place) => {
                // Handle references
                if let Some(lock_def_id) =
                    self.resolve_place_to_lock_object(borrowed_place, lock_map, dependency_map)
                {
                    lock_map.insert(place.local, lock_def_id);
                }
                if let Some(guard_def_id) =
                    self.resolve_place_to_lockguard(borrowed_place, guard_map, dependency_map)
                {
                    guard_map.insert(place.local, guard_def_id);
                }
                // Update data dependency graph
    ...
    fn resolve_lock_object_from_args(
        &self,
    ...
    fn resolve_operand_to_lock_object(
        &self,
        operand: &Operand<'tcx>,
        lock_map: &HashMap<Local, DefId>,
        dependency_map: &HashMap<Local, HashSet<Local>>,
    ...
    fn resolv_operand_to_lockguard(
        &self,
        operand: &Operand<'tcx>,
    ...
    pub fn print_lockset_analysis_results(&self) {
        ...
        rap_info!(
            "==== Lockset Analysis Results End ({} non-trivial functions) ====",
            lock_count
        );
    }
}
